...
module.exports = {
  ...
  category: "Settings",
  usage:
    "<role|messagechannel|message{user|server|membercount}>|[--info|--settings] <<role|ID>|<channel|ID>|<message>>",
  cooldown: 5,
  ...
  async execute(client, message, args) {
    ...
    if (args[0].toLowerCase() == "role") {
      ...
      message.channel.send(
        client.embedSuccess(
          message,
          `Successfully set ${role} as the verification \`role\`.`
        )
      );
      ...
      message.channel.send(
        client.embedSuccess(
          message,
          `Successfully set ${channel} as the verification \`messagechannel\`.`
        )
      );
      ...
      if (text.length > 1024) {
        return message.channel.send(
          client.embedError(
            message,
            `The verification message cannot be longer than 1024 characters.\nYou provided (${text.length}/1024)`
          )
        );
      }
      await guildVerificationSystemModel.findOneAndUpdate(
      ...
      message.channel.send(
        client.embedSuccess(
          message,
          "Successfully set the verification `message`."
        )
      );
      ...
      if (verificationData) {
        ...
        messageChannel = message.guild.channels.cache.get(
          verificationData.messageChannelId
        );
        if (!messageChannel) {
      ...
      if (!text) text = "No text set";
      if (channelData) {
        verificationChannel = message.guild.channels.cache.get(
          channelData.acceptChannel
        );
      }
      if (!verificationChannel)
        verificationChannel = "No verification channel set";
