  ...
  async execute(client, message, args) {
    ...
    if (!channel) {
      return message.channel.send(
        client.embedError(
          message,
          "The reports channel for this server does not exist anymore."
        )
      );
    }
    ...
    if (!reportChannelPermissions.has("SEND_MESSAGES")) {
      return message.channel.send(
        client.embedError(
          message,
          "I do not have permissions to send messages in the reports channel."
        )
      );
    }
    if (!reportChannelPermissions.has("ATTACH_FILES")) {
      return message.channel.send(
        client.embedError(
          message,
          "I do not have permissions to send embeds in the reports channel."
        )
      );
    }
