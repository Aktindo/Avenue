  ...
  async execute(client, message, args) {
    if (!args[0]) {
      return message.channel.send(
        client.embedError(
          message,
          "Please provide a time! See `[prefix]help slowmode` for more information on how to use the syntax."
        )
      );
    }
    if (isNaN(args[0])) {
    ...
    message.channel.send(
      client.embedSuccess(
        message,
        `Successfully set the slowmode of ${message.channel} to \`${args[0]}\` seconds!`
      )
    );
  },
};
