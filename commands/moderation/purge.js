...
module.exports = {
    ...
    async execute(client, message, args) {
        ...
        if (args[0].toLowerCase() == 'all') {
        ...
        else if (args[0].toLowerCase() == 'matchtext') {
            let text = args[1]
            if (!text) {
                return message.channel.send(
            ...
            await message.channel.messages.fetch({limit: parseInt(amount)}).then(messages => {
                const matchedTextMessages = messages.filter(message => (message.content == text))
            ...
            await message.channel.messages.fetch({limit: parseInt(amount)}).then(messages => {
                const notMatchedTextMessages = messages.filter(message => (message.content != text))
