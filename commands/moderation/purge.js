module.exports = {
  ...
  cooldown: 5,
  usage:
    "<type(all|bots|humans|matchedText|notMatchedText)> <text?if(matchedText|notMatchedText)> <amount?number>",
  ...
  async execute(client, message, args) {
    ...
    if (args[0].toLowerCase() == "all") {
        ...
        .then((messages) => {
          ...
          message.channel.send(
            client.embedSuccess(
              ...
              `Successfully purged ${
                parseInt(amount) + 1
              } messages from this channel with no filters.`
            )
          );
        });
      ...
      await message.channel.messages
        ...
        .then((messages) => {
          ...
          message.channel.send(
            client.embedSuccess(
              message,
              `Successfully purged ${botMessagesLength} messages from this channel with filter - \`Delete only bot messages\``
            )
          );
        });
      ...
      await message.channel.messages
        ...
        .then((messages) => {
          ...
          message.channel.send(
            client.embedSuccess(
              message,
              `Successfully purged ${humanMessagesLength} messages from this channel with filter - \`Delete only human messages\``
            )
          );
        });
      ...
      await message.channel.messages
        .fetch({ limit: parseInt(amount) })
        .then((messages) => {
          const matchedTextMessages = messages.filter((msg) =>
          ...
          message.channel.send(
            client.embedSuccess(
              message,
              `Successfully purged ${matchedTextMessagesLength} messages from this channel with filter - \`Delete only matched text messages\``
            )
          );
        });
      ...
      await message.channel.messages
        .fetch({ limit: parseInt(amount) })
        .then((messages) => {
          const notMatchedTextMessages = messages.filter(
            (msg) => !msg.content.includes(text)
          );
          const notMatchedTextMessagesLength = notMatchedTextMessages.array()
            .length;
          message.channel.bulkDelete(notMatchedTextMessages, true);
          message.channel.send(
            client.embedSuccess(
              message,
              `Successfully purged ${notMatchedTextMessagesLength} messages from this channel with filter - \`Delete only not matched text messages\``
            )
          );
        });
    } else {
      message.channel.send(client.embedError(message, "Invalid syntax given!"));
    }
  },
};
