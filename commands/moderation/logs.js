...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const warningLogs = await warningModel
      .find({
    ...
    const banLogs = await banSystemModel
      .find({
        guildId: message.guild.id,
        userId: user.id,
      })
    ...
    const kickLogs = await kickSystemModel
      .find({
        guildId: message.guild.id,
        userId: user.id,
      })
    ...
    const reportLogs = await reportSystemModel
      .find({
        guildId: message.guild.id,
        userId: user.id,
      })
    ...
    const embed = new MessageEmbed()
      .setAuthor(
        `All logs for ${user.user.username}`,
