...
module.exports = {
	...
	async execute(client, message) {
		...
		const verificationChannel = message.guild.channels.cache.get(channelData.acceptChannel);
		if (!verificationChannel) return;
		const role = message.guild.roles.cache.get(verificationData.roleId);
		if (!role) return;
		const messageChannel = message.guild.channels.cache.get(verificationData.messageChannelId);
